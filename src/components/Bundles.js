...
const Bundles = ({bundles}) => {
    ...
    useEffect(() => {
        window.addEventListener('scroll', () => {
            ...
            window.scrollY > 2100 ? setEffect(true) : setEffect(false)
        })
  ...
  return (
    <div className='flex flex-col md:flex-row justify-between gap-[30px]'>
        {bundles.map((bundle,index) => {
            ...
            return (
            <div className={`${effect ? 'slide-in-fwd-left' : 'w-full'} w-full lg:max-w-1/3 bg-white shadow-primary text-center h-[560px] rounded-[60px] py-12 px-8`} key={index}>
                {/*price */}
